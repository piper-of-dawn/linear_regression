...
def get_ohlc_data(tickers, start_date="2017-01-01", end="2023-01-01"):
    ohlc_data = {}
    for ticker in tickers:
        try:
            stock_data = yf.download(ticker, start=start_date, end=end)
...
class TimeSeries:
    def __init__(self, ticker: str, date: str = "2017-01-01", end: str = "2023-01-01"):
        self.data = get_ohlc_data([ticker], date, end)[ticker].reset_index()
    ...
    def lag_column(self, column, skip_lags, n_lags):
        ...
        self.data = self.data.with_columns(
            [
                pl.col(column).shift(lag).alias(f"{column}_lag{lag}")
                for lag in range(skip_lags, n_lags + 1)
            ]
        ).to_pandas()
        return self
    ...
    def pca(self, explained_variance_threshold, target):
        ...
        transformed_features = pca.transform(self.data.drop(target, axis=1).to_numpy())[
            :, :num_components
        ]
        self.data = transformed_features
        print(
            f"Number of components explaining at least {explained_variance_threshold*100}% of the variation:",
            num_components,
        )
    ...
    def train_test_split(self, test_size, target):
        if isinstance(self.data, pd.DataFrame):
            self.training_data = train_test_split(
                data=self.data, test_size=test_size, y_name=target
            )
        else:
            self.training_data = train_test_split_numpy(
                data=self.data, test_size=test_size
            )
        return self
    ...
    def encode_labels(self, columns_start_with: str):
        ...
        self.data = encode_labels(
            ...
            [
                ...
                if column.startswith(columns_start_with)
            ],
        )
        return self
    ...
    def shift_column(self, column, n_shifts, drop_nulls):
        if n_shifts > 0:
            ...
            suffix = "Ahead"
        self.data[f"{column}_{suffix}_{abs(n_shifts)}"] = self.data[column].shift(
            n_shifts
        )
        if drop_nulls:
            self.data = self.data.dropna()
        ...
        if min_max_scale:
            ...
            if which_data == "train":
                return np.expand_dims(
                    scaler_input.transform(self.training_data.x_train), axis=1
                )
            else:
                return np.expand_dims(
                    scaler_input.transform(self.training_data.x_test), axis=1
                )
        if which_data == "train":
    ...
    def plot(self):
        return plot_multiline_chart(
            [(self.date, self.close_price, self.ticker)],
            f"Daily Close ({self.ticker})",
            "Date",
            "Close",
        )
...
def min_max_scale(time_series, which_data="x"):
    ...
    if which_data == "x":
        ...
        return scaler_input.transform(
            time_series.training_data.x_train
        ), scaler_input.transform(time_series.training_data.x_test)
    else:
        y_train = time_series.training_data.y_train.reshape(
            len(time_series.training_data.y_train), 1
        )
        y_test = time_series.training_data.y_test.reshape(
            len(time_series.training_data.y_test), 1
        )
...
def calculate_summary_statistics(time_series):
    ...
    # Approximate Entropy
    approximate_entropy = feature_calculators.approximate_entropy(
        time_series, m=30, r=0.7 * np.std(time_series)
    )

    # ADF Statistic (Augmented Dickey-Fuller test)
    adf_statistic = feature_calculators.augmented_dickey_fuller(
        time_series, param=[{"attr": "pvalue", "autolag": "AIC"}]
    )
...
def inverse_transform_predictions(ticker: TimeSeries, preds: np.ndarray):
    scaler_input = MinMaxScaler(feature_range=(-1, 1))
    scaler_input.fit(
        ticker.training_data.y_train.reshape(len(ticker.training_data.y_train), 1)
    )
...
def get_actual_and_predicted(ticker: TimeSeries, model):
    ...
    preds = model.model.predict(
        ticker.get_reshaped_X_for_LSTM(which_data="test", min_max_scale=True)
    ).flatten()
...
def plot_actual_vs_predicted(ticker: TimeSeries, model, title):
    ...
    plot_multiline_chart(
        [
            (idx, inverse_transform_predictions(ticker, preds), "Predicted"),
            (idx, true, "Actual"),
        ],
        title,
        "Index",
